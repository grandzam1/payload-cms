 ...
 * using the Neon pooler hostname and stripping channel_binding.
 *
...
const vercel = process.platform === 'win32' ? '.\\node_modules\\.bin\\vercel.CMD' : './node_modules/.bin/vercel'
...
for (const envName of ['production', 'preview', 'development']) {
  ...
  const add = spawnSync(vercel, ['env', 'add', 'DATABASE_URL', envName, '--yes'], {
    input: pooled,
    encoding: 'utf8',
    shell: true,
  })
  const out = `${add.stdout || ''}${add.stderr || ''}`
  console.log(envName, /Added|Updated|Error|already/i.test(out) ? out.match(/Added|Updated|Error|already[^\n]*/i)?.[0] : add.status)
}

// Also write local .env.neon DATABASE_URL for migrate
const updated = text.includes('DATABASE_URL=')
  ? text.replace(/^DATABASE_URL=.*$/m, `DATABASE_URL="${pooled}"`)
  : `DATABASE_URL="${pooled}"\n${text}`
